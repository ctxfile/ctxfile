...
9. **Update Notion after every significant merge to `main`.** Append a dated one-liner (2 to 5 bullets max) to **[ctxfile — Business HQ](https://app.notion.com/p/39b0944c074381169b9bca4c37bf190f)** (page id `39b0944c-0743-8116-9b9b-ca4c37bf190f`) via the Notion MCP tools, so the workspace always reflects what actually shipped. Write the *status*, not the code: version published, what a user can now do, what is live where, what is still pending. No diffs, no file lists, no internal reasoning. Skip it only for trivial merges (typo, lint, dependency bump). Secrets, tokens, and vault credentials never go in Notion.
